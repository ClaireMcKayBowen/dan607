...
All examples provided in these callout boxes are written and verbal examples from within the past year (since 2023), with most from this year (2024). Although there is no sensitive information in the emails, I change the names and titles.
...
This next example is how a colleague introduced himself to me. Even though we had worked together previously, several months had passed. He didn't presume that I remembered him and made sure to clearly state who he is and our prior connection.
...
In the following example, staff from the a state's Department of Health contacted me regarding alternative data privacy methods aimed at avoiding the suppression or removal of segments of public health data. The forward momentum is opening the door for further questions.
...
Subteam A requires additional funds beyond what remains allocated in the Subteam A project code (Funder A Year 2) to cover their time spent presenting the tool demo to our project group and Funder A. This includes the time needed to create and prepare the presentation. *[punchline]*

One potential solution I propose is to request those currently charging to the Funder A code (a subset of the Subteam C) to transition to the Funder B code instead. The remaining balance for the Funder A code stands at $7,472 and would be then used by Subteam A. To be honest, this would be a good option from an admin standpoint as it would narrow down the number of individuals charging the Funder A code to just the Subteam A (Subteam B has stopped charging already) as we wind down towards March 31 (the end of the code). *[outstanding items]*

If we make this change, the following would be the new code assignments for the Subteam C until the Funder A Year 3 Codes are activated: 

- Funder B - [List of five names]
- Funder C - [List of six names]
